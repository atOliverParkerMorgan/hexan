...
export default class Map implements MapInterface {
    ...
    generateContinent(seed_x: number, seed_y: number, continent_size: number, continent_name: string): void {
        ...
        for (let i = 0; i < continent_size;) {
            ...
            let random_continent_node = current_continent.getRandomNodeOfType(Utils.GRASS);
            if (random_continent_node == null) continue;
            let random_neighbour_node = random_continent_node.getRandomNeighbourOfType(Utils.OCEAN);

            if (random_neighbour_node == null) {
                current_continent.changeNodeTo(random_continent_node, Utils.FOREST);
            } else {
        ...
        for (const node of current_continent.grass_nodes) {
            if (!node.isCoast()) {
                current_continent.changeNodeTo(node, Utils.FOREST);
            }
        }
        ...
        for (let i = 0; i <= number_of_mountain_ranges; i++) {
            ...
            if (i === 0) this.generateMountains(seed_x, seed_y, number_of_mountain_ranges, current_continent, mountain_type);
                ...
                let random_grass_node = current_continent.getRandomNodeOfType(Utils.GRASS);
    ...
    generateMountains(seed_x: number, seed_y: number, size: number, current_continent: ContinentInterface, mountain_type: number): void {
    ...
    generateRiver(continent: ContinentInterface): void {
